...
def non_max_suppression1(prediction, conf_thres_list=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
                        labels=()):
    ...
    xc = prediction[..., 4] < 0
    for idx, conf_thres in enumerate(conf_thres_list):
        xc = xc | ((max_idx == idx) & (prediction[..., 4] > conf_thres))

    '''
    conf0, j0 = prediction[:, :, 5:].max(2, keepdim=True)
